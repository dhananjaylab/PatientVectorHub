...
class TestRetrieve:
    @pytest.mark.asyncio
    async def test_embeds_query_then_searches_store(self, mock_vector_store):
        ...
        with (
            ...
            patch.object(retriever, "get_store", return_value=mock_vector_store) as mocked_get_store,
        ):
    ...
    @pytest.mark.asyncio
    async def test_filters_passed_through_unchanged(self, mock_vector_store):
        from src import retriever

        filters = {"document_types": ["lab_result"]}
        with (
            patch.object(retriever, "embed_query", new=AsyncMock(return_value=[0.1])),
            patch.object(retriever, "get_store", return_value=mock_vector_store),
        ):
    ...
    @pytest.mark.asyncio
    async def test_default_top_k_is_ten(self, mock_vector_store):
        from src import retriever
