#!/usr/bin/env python3
"""Copy rag-engine tests and remove sys.path manipulation."""
...
def copy_test_file(test_name: str, test_type: str) -> bool:
    ...
    if test_type == 'unit':
        ...
        dst = REPO_ROOT / 'rag-engine' / 'tests' / 'unit' / test_name
    else:
        src = REPO_ROOT / 'tests' / 'integration' / test_name
        dst = REPO_ROOT / 'rag-engine' / 'tests' / 'integration' / test_name
...
def main():
    print("Copying rag-engine tests...\n")
    
    # Ensure directories exist
    (REPO_ROOT / 'rag-engine' / 'tests' / 'unit').mkdir(parents=True, exist_ok=True)
    (REPO_ROOT / 'rag-engine' / 'tests' / 'integration').mkdir(parents=True, exist_ok=True)
