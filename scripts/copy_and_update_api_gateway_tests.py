#!/usr/bin/env python3
"""Copy api-gateway tests and remove sys.path manipulation."""
...
def copy_test_file(test_name: str, test_type: str) -> bool:
    ...
    if test_type == 'unit':
        ...
        dst = REPO_ROOT / 'api-gateway' / 'tests' / 'unit' / test_name
    else:
        src = REPO_ROOT / 'tests' / 'integration' / test_name
        dst = REPO_ROOT / 'api-gateway' / 'tests' / 'integration' / test_name
...
def main():
    print("Copying api-gateway tests...\n")
    
    # Ensure directories exist
    (REPO_ROOT / 'api-gateway' / 'tests' / 'unit').mkdir(parents=True, exist_ok=True)
    (REPO_ROOT / 'api-gateway' / 'tests' / 'integration').mkdir(parents=True, exist_ok=True)
