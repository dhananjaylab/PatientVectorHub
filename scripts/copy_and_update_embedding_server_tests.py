#!/usr/bin/env python3
"""Copy embedding-server tests and remove sys.path manipulation."""
...
INTEGRATION_TESTS = [
    # No integration tests for embedding-server yet
]
...
def copy_test_file(test_name: str, test_type: str) -> bool:
    ...
    if test_type == 'unit':
        ...
        dst = REPO_ROOT / 'embedding-server' / 'tests' / 'unit' / test_name
    else:
        src = REPO_ROOT / 'tests' / 'integration' / test_name
        dst = REPO_ROOT / 'embedding-server' / 'tests' / 'integration' / test_name
...
def main():
    print("Copying embedding-server tests...\n")
    
    # Ensure directories exist
    (REPO_ROOT / 'embedding-server' / 'tests' / 'unit').mkdir(parents=True, exist_ok=True)
    (REPO_ROOT / 'embedding-server' / 'tests' / 'integration').mkdir(parents=True, exist_ok=True)
