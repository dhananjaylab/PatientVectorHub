...
from ..observability import scheduled_task_duration_seconds, scheduled_task_runs_total
...
def _track_scheduled_task(task_name: str):
    """Phase 10 / ADR-017. Shared by both tasks below rather than
    duplicating the same try/except/timing boilerplate in each --
    records pvh_scheduled_task_duration_seconds and
    pvh_scheduled_task_runs_total{status="success"|"failure"}. Re-raises
    whatever the task body raised after recording "failure", so Celery's
    own task-failure handling (logging, any configured retry policy)
    still sees the real exception unchanged."""
    ...
    try:
        ...
        scheduled_task_duration_seconds.labels(task_name=task_name).observe(
            time.perf_counter() - start
        )
        scheduled_task_runs_total.labels(task_name=task_name, status=status).inc()


@celery_app.task(name="pvh.cleanup_completed_jobs")
def cleanup_completed_jobs() -> None:
    with _track_scheduled_task("pvh.cleanup_completed_jobs"):
...
def requeue_stale_documents() -> None:
    ...
    with _track_scheduled_task("pvh.requeue_stale_documents"):
        for tenant_id in get_all_tenant_ids():
            with get_tenant_sync_session(tenant_id) as db:
                stale = db.execute(
                    text(
