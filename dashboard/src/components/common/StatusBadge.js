...
export function StatusBadge({ status, label }) {
    ...
    return (_jsxs("span", { className: "status-badge", style: { color }, children: [_jsx("span", { className: PULSE.has(status) ? 'status-dot status-dot-pulse' : 'status-dot', style: { background: color } }), (label ?? status).replace(/_/g, ' ').toUpperCase()] }));
}
